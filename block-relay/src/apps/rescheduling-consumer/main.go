...
type EnvVars struct {
	DatabaseUrl    string `validate:"required,gt=0" env:"RESCHEDULING_CONSUMER_DATABASE_URL,required"`
	RedisStreamUrl string `validate:"required,gt=0" env:"RESCHEDULING_CONSUMER_REDIS_STREAM_URL,required"`
	ConsumerName   string `validate:"required,gt=0" env:"RESCHEDULING_CONSUMER_NAME,required"`
	MaxPoolSize    int    `validate:"required,gt=0" env:"RESCHEDULING_CONSUMER_MAX_POOL_SIZE,required"`
	BlockTimeoutMs int    `validate:"required,gte=0" env:"RESCHEDULING_CONSUMER_BLOCK_TIMEOUT_MS,required"`
}
