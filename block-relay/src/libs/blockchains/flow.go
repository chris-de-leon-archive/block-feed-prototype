...
func NewFlowBlockchain(id ChainID, url string) (IBlockchain, error) {
	validChainIDs := map[ChainID]ChainID{
		FLOW_TESTNET: FLOW_TESTNET,
		FLOW_MAINNET: FLOW_MAINNET,
	}
	if _, exists := validChainIDs[id]; !exists {
		return nil, fmt.Errorf("\"%s\" is an invalid chain ID for Flow client", id)
	}
...
func (blockchain *FlowBlockchain) Close() {
