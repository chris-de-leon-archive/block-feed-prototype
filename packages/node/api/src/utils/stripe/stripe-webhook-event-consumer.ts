import { DatabaseVendor, StripeVendor, RedisVendor } from "@block-feed/vendors"
...
export class StripeWebhookEventConsumer {
  private constructor(
    ...
    private readonly stripeVendor: StripeVendor,
    private readonly redisVendor: RedisVendor,
    private readonly dbVendor: DatabaseVendor,
  ...
  static async build(
    ...
    try {
      await redisVendor.client.xgroup(
        "CREATE",
  ...
  async *consume(controller: AbortController, blockMs = 0) {
    ...
    for await (const _ of setInterval(1, undefined)) {
      ...
      // ]
      //
      const streams = await this.redisVendor.client.xreadgroup(
        "GROUP",
      ...
      for (const stream of streams) {
        ...
        if (
          streamName !== STRIPE_CONSTANTS.STREAMING.WEBHOOK_EVENT_STREAM_NAME
        ) {
          continue
        }
        ...
        if (stripeEvents.size !== 0) {
          ...
          await this.redisVendor.client.xackdel(
            STRIPE_CONSTANTS.STREAMING.WEBHOOK_EVENT_STREAM_NAME,
            this.consumerGroupName,
  ...
  private async handleCustomerDeleted(
    ...
    const subscriptions = await this.stripeVendor.client.subscriptions.list({
    ...
    const metadata = zStripeSubscriptionMetadata.parse(sub.metadata)
    await Promise.allSettled([
      this.stripeCheckoutSessCache.invalidate(metadata.userId),
      this.dbVendor.drizzle.transaction(async (tx) => {
        await tx
  ...
  private async invalidateCachedSubscription(
    ...
    const getMetadata = async () => {
      if (event.type === "checkout.session.completed") {
        return zStripeCheckoutSessionMetadata.parse(
      ...
      if (
        ...
        return zStripeSubscriptionMetadata.parse(event.data.object.metadata)
      }
      ...
      const sub =
        typeof subscription === "string"
          ? await this.stripeVendor.client.subscriptions.retrieve(subscription)
          : subscription

      return zStripeSubscriptionMetadata.parse(sub.metadata)
    }
