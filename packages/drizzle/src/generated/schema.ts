...
export const webhookClaim = mysqlTable(
  "webhook_claim",
  {
    id: varchar("id", { length: 36 }).notNull(),
    createdAt: datetime("created_at", { mode: "string" })
...
export const webhookLocation = mysqlTable(
  "webhook_location",
  {
    id: varchar("id", { length: 36 }).notNull(),
    createdAt: datetime("created_at", { mode: "string" })
      .default(sql`CURRENT_TIMESTAMP`)
      .notNull(),
    webhookClaimId: varchar("webhook_claim_id", { length: 36 })
      .notNull()
      .references(() => webhookClaim.id, { onDelete: "cascade" }),
    webhookNodeId: varchar("webhook_node_id", { length: 36 })
      .notNull()
      .references(() => webhookNode.id, { onDelete: "cascade" }),
    webhookId: varchar("webhook_id", { length: 36 })
      .notNull()
      .references(() => webhook.id, { onDelete: "cascade" }),
  },
  (table) => {
    return {
      ...
      webhookLocationId: primaryKey({
        columns: [table.id],
        name: "webhook_location_id",
      }),
      webhookId: unique("webhook_id").on(table.webhookId),
      webhookId2: unique("webhook_id_2").on(
        table.webhookId,
        table.webhookNodeId,
      ),
    }
...
export const webhookNode = mysqlTable(
  "webhook_node",
  {
    id: varchar("id", { length: 36 }).notNull(),
    createdAt: datetime("created_at", { mode: "string" })
