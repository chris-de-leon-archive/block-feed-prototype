CREATE FUNCTION pg_temp.update_api_role(
  schma TEXT,
  uname TEXT,
  pword TEXT
) RETURNS VOID AS
...
DECLARE
  ...
  queries TEXT[] := ARRAY[
    ...
    format('GRANT SELECT, INSERT, UPDATE ON TABLE %I."customer" TO %I;', schma, uname),

    -- blockchain
    format('REVOKE ALL PRIVILEGES ON %I."blockchain" FROM %I;', schma, uname),
    format('GRANT SELECT ON TABLE %I."blockchain" TO %I;', schma, uname),

    -- webhook
    format('REVOKE ALL PRIVILEGES ON %I."webhook" FROM %I;', schma, uname),
    format('GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE %I."webhook" TO %I;', schma, uname),
    ...
    format('GRANT SELECT, INSERT ON TABLE %I."webhook_job" TO %I;', schma, uname),
    ...
    format('GRANT SELECT ON TABLE %I."block_cache" TO %I;', schma, uname)
  ];
BEGIN
...
SELECT pg_temp.update_api_role(:'schema', :'uname', :'pword');
