...
for chain in ${chains[@]}; do
	bash ./scripts/frontend/blockchains/insert.sh "$chain"
done
bash ./scripts/frontend/blockchains/list.sh
...
for ((i = $2; i <= $3; i++)); do
	...
	bash ./scripts/frontend/webhooks/insert.sh \
		"http://localhost:$i" \
		"$1" \
		"$rand_chain"
done
bash ./scripts/frontend/webhooks/list.sh
