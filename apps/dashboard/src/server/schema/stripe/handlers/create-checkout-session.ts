...
export const handler = async (
  ...
  const session = await ctx.providers.stripe.client.checkout.sessions.create({
    ...
    subscription_data: {
      ...
      ...(result.stripeCustomerId == null
        ...
        : {}),
      metadata: ctx.providers.stripe.makeMetadata({ userId }),
    },
    metadata: ctx.providers.stripe.makeMetadata({ userId }),
    automatic_tax: {
      enabled: true,
  ...
  return await ctx.providers.mysql.drizzle.transaction(async (tx) => {
...
const resolveCheckoutSession = async (ctx: GraphQLAuthContext) => {
  ...
  const existingSession =
    await ctx.providers.mysql.drizzle.query.checkoutSession.findFirst({
      where: eq(schema.checkoutSession.customerId, userId),
    })
  ...
  if (sess.status === "complete") {
    ...
    // database.
    const customer = await ctx.providers.stripe.extractStripeCustomer(sess)
    ...
    const sub = await ctx.providers.stripe.extractStripeSubscription(sess)
