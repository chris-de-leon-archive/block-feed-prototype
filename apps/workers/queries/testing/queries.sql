...
VALUES (sqlc.arg('id'), sqlc.arg('created_at'), sqlc.arg('is_queued'), sqlc.arg('is_active'), sqlc.arg('url'), sqlc.arg('max_blocks'), sqlc.arg('max_retries'), sqlc.arg('timeout_ms'), sqlc.arg('customer_id'), sqlc.arg('blockchain_id'));

-- name: CreateWebhookNodes :execrows
INSERT INTO `webhook_node` (`id`, `created_at`, `url`, `blockchain_id`) 
VALUES (sqlc.arg('id'), sqlc.arg('created_at'), sqlc.arg('url'), sqlc.arg('blockchain_id'));
