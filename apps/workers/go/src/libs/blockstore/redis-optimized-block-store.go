...
func (blockStore *RedisOptimizedBlockStore) StartFlushing(ctx context.Context, chainID string, opts RedisOptimizedBlockStoreFlushOpts) error {
	...
	flushCache := func() error {
		blocks, err := blockStore.redisStore.GetEarliestBlocks(ctx, chainID, int64(opts.Threshold))
		if err != nil {
			return err
		}
		if len(blocks) >= opts.Threshold {
			...
			if err := blockStore.redisStore.DeleteBlocks(ctx, chainID, blocks); err != nil {
				return err
			}
