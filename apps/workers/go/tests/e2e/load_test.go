...
func TestLoad(t *testing.T) {
	...
	store, err := testutils.NewRedisOptimizedBlockStore(t, ctx, config)
	if err != nil {
		t.Fatal(err)
	}

	// Creates a flow block streamer service
	flowBlockStreamer, err := testutils.NewFlowBlockStreamer(t, ctx, config)
	if err != nil {
		t.Fatal(err)
	}
	...
	for shardID := range TEST_SHARDS {
		for replicaNum := range WEBHOOK_CONSUMER_REPLICAS_PER_SHARD {
			...
			if err != nil {
				...
				webhookConsumers[(shardID*WEBHOOK_CONSUMER_REPLICAS_PER_SHARD)+replicaNum] = webhookConsumer
			}
		}
	}
	...
	webhooks := testutils.CreateManyWebhooks(
		TEST_NUM_WEBHOOKS,
		server.URL,
		WEBHOOK_MAX_BLOCKS,
		WEBHOOK_MAX_RETRIES,
	...
	if err := testutils.InsertManyWebhooks(ctx,
		cMySqlDB.Conn.Url,
		testutils.MYSQL_DEFAULT_CONN_POOL_SIZE,
		customerID,
		config,
	...
	if err := testutils.ActivateManyWebhooks(ctx, cRedisCluster.Conn.Url, webhooks); err != nil {
		t.Fatal(err)
	}
	...
	for _, consumer := range webhookConsumers {
		c := consumer
		eg.Go(func() error { return c.Run(timeoutCtx) })
	}
