...
func (service *BlockPoller) pushBlocks(ctx context.Context, blocks []blockstore.BlockDocument) error {
	...
	data, err := messaging.NewBlockStreamMsg(service.chain.ID(), blocks).MarshalBinary()
	if err != nil {
		return err
	}
...
func (service *BlockPoller) getLastProcessedHeight(ctx context.Context) (*uint64, error) {
	// Gets the last element pushed to the block stream
	elems, err := service.redisClient.XRevRangeN(ctx, constants.BLOCK_STREAM, "+", "-", 1).Result()
	if err != nil {
		return nil, err
	}
	...
	if len(elems) > 0 {
		parsedMsg, err := messaging.ParseMessage[messaging.BlockStreamMsgData](elems[0])
		if err != nil {
			return nil, err
		} else {
	...
	// If there's nothing in the block stream, then check the block flush stream.
	// If the block flush stream has nothing in it, wait for the block processor
	// to add something to it on startup. If the block flush stream already has
	// an element, use it to get the height.
	for {
		select {
		case <-ctx.Done():
			return nil, nil
