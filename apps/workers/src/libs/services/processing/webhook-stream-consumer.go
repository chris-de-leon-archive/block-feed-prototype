...
type (
	WebhookStreamConsumerOpts struct {
		ConsumerName string
		Concurrency  int
	}

	WebhookStreamConsumerParams struct {
		BlockStore    blockstore.IBlockStore
		RedisCluster  *redicluster.RedisCluster
		WebhookStream *streams.WebhookStream
		Opts          *WebhookStreamConsumerOpts
	}

	WebhookStreamConsumer struct {
		blockStore    blockstore.IBlockStore
		redisCluster  *redicluster.RedisCluster
		webhookStream *streams.WebhookStream
		opts          *WebhookStreamConsumerOpts
...
func NewWebhookStreamConsumer(params WebhookStreamConsumerParams) *WebhookStreamConsumer {
	return &WebhookStreamConsumer{
...
func (service *WebhookStreamConsumer) Run(ctx context.Context) error {
...
func (service *WebhookStreamConsumer) handleMessages(
	ctx context.Context,
	msgs []*messaging.ParsedStreamMessage[*messaging.WebhookStreamMsgData],
	isBacklogMsg bool,
	...
	// Gets the webhook data - if it is no longer exists then this
	// message will be ACK'd + deleted and we can exit early
	webhook, err := service.redisCluster.Webhooks.Get(ctx, service.webhookStream.ShardNum, msg.Data.WebhookID)
	if webhook == nil {
		return service.webhookStream.XAck(ctx, msg.ID, nil)
	}
	if err != nil {
		return err
	}
	...
	if isBacklogMsg {
		...
		if pendingMsg.RetryCount >= webhook.MaxRetries {
			// TODO: is there a better way to handle repeated errors?
			return service.webhookStream.XAck(
				ctx,
				msg.ID,
	...
	nextBlockHeight := msg.Data.BlockHeight
