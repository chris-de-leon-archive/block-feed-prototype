...
func TestRedisOptimizedBlockStore(t *testing.T) {
	...
	blockStore := blockstore.NewRedisOptimizedBlockStore(
	...
	t.Run("Blocks Flushed", func(t *testing.T) {
		...
		if count != 0 {
			t.Fatalf("Expected %d elements to be flushed from the cache, but %d still remain", len(blocks), count)
		}
	})
	...
	t.Run("Put Extra Block", func(t *testing.T) {
		...
		if count != 1 {
			t.Fatalf("Expected %d elements to be in the store but got %d", 1, count)
		}
	})
